...
*/
    ...
    while ($row = $db->fetch()) {
      ...
      // check node fees filter 0.001
      if ($json->feefilter <= 0.001){ $t001++; };

      // check if the node has fees at 0.001, if not change the marker to blue
      if ($class == "yhellowDogeMiner"){ if ($json->feefilter != "0.001"){ $class = "blueDogeMiner"; }; };
...
?>
    <div class="col">
      <button type="button" class="btn btn-primary">
        Node FeeFilter @ 0.001: <span class="badge bg-light" style="color: #000000"><?php echo $t001; ?></span>
      </button>
    </div>